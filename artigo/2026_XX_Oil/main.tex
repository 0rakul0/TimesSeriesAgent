...
\begin{IEEEkeywords}
Financial time series forecasting, event retrieval, news-aware forecasting, hybrid models, semantic similarity, residual correction.
\end{IEEEkeywords}
...
\begin{itemize}
    \item uma biblioteca auditável de eventos financeiros estruturados a partir de notícias, com contexto textual, motivos associados e propagação observada de impacto após o evento;
...
Três modelos quantitativos foram treinados como baselines: LSTM, Autoencoder recorrente e Transformer. Todos recebem como entrada séries sincronizadas do ativo alvo e do Brent e produzem previsões base de preço. Sobre essa previsão, o método híbrido adiciona duas camadas.
...
\begin{tabular}{@{}C{0.12\columnwidth} L{0.23\columnwidth} C{0.18\columnwidth} C{0.20\columnwidth} C{0.12\columnwidth}@{}}
\toprule
\textbf{Ativo} & \textbf{Modelo} & \specialcell{\textbf{RMSE}\\\textbf{Base}} & \specialcell{\textbf{RMSE}\\\textbf{Híbrido}} & \textbf{Ganho} \\
\midrule
...
Como complemento interpretativo, a Tabela~\ref{tab:topicos_impacto} resume os cinco assuntos com maior alteração absoluta de preço no horizonte futuro entre os clusters com pelo menos cinco eventos. Diferentemente do ajuste preditivo principal, que opera no horizonte D0--D4, essa tabela usa D1--D5 para enfatizar a propagação posterior ao dia do evento e evitar que o choque inicial domine sozinho a interpretação. O critério usado foi a soma absoluta da trajetória média posterior nesse horizonte, preservando o sinal médio observado. Assim, a tabela não mede a polaridade semântica intrínseca do assunto, mas sim o comportamento médio dos preços após eventos daquele tipo.
...
\resizebox{\columnwidth}{!}{%
\begin{tabular}{@{}L{0.50\columnwidth} C{0.12\columnwidth} C{0.07\columnwidth} R{0.17\columnwidth}@{}}
\toprule
\specialcell{\textbf{Assunto}\\\textbf{representativo}} & \textbf{Ativo} & \textbf{$n$} & \specialcell{\textbf{Impacto}\\\textbf{D1--D5}} \\
\midrule
...
A principal ameaça à validade interna decorre da dependência de serviços externos para estruturação de notícias e geração de embeddings. Mesmo com prompts controlados, JSON estruturado e \textit{fallback} de busca, ainda existe variação induzida pelo modelo de linguagem e pela disponibilidade de conteúdo recuperado. Além disso, a abordagem não busca demonstrar causalidade econômica formal. O que o método mostra é que certos eventos comparáveis carregam informação útil para ajuste preditivo sob restrição temporal.
